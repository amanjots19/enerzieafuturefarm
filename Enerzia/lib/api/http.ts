 ...
 * header. POST /auth/otp/verify also returns 401 (wrong OTP), so a
 * header-gated check is the only way to avoid logging the shopper out when
 * they mistype their code.
 */
